...
// Function currently just calls sock352_init2. Don't really know what this function is supposed to be for.
...
int sock352_accept(int _fd, sockaddr_sock352_t *addr, int *len) {
    ...
    // Get the correct port for the remote host.
    if (uport > 0) {
...
// Function is responsible for handling the 4-way TCP teardown.
int sock352_close(int fd) {
    ...
    if (socket->type == SOCK352_CLIENT || socket->type == SOCK352_ACCEPT) {
        ...
        // Give the sending thread our FIN packet as a final packet, and set the local FIN flag.
...
void *send_queue(void *sock) {
    ...
    while (!socket->lfin) {
        ...
        // printf("send_queue: About to send sequence number %ld with %d bytes of data...\n", chunk->header.sequence_no, chunk->header.payload_len);
